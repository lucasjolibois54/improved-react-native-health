...
- (void)dietary_getPolyunsaturatedFatSamples:(NSDictionary *)input callback:(RCTResponseSenderBlock)callback {
    ...
    [self fetchQuantitySamplesOfType:type unit:unit predicate:predicate ascending:ascending limit:limit completion:^(NSArray *results, NSError *error) {
        if (results) {
            ...
            callback(@[RCTMakeError(@"An error occurred while retrieving polyunsaturated fat", error, nil)]);
        }
    }];
}
...
- (void)dietary_getSugarSamples:(NSDictionary *)input callback:(RCTResponseSenderBlock)callback {
    ...
    [self fetchQuantitySamplesOfType:type unit:unit predicate:predicate ascending:ascending limit:limit completion:^(NSArray *results, NSError *error) {
        if (results) {
            ...
            callback(@[RCTMakeError(@"An error occurred while retrieving sugar", error, nil)]);
        }
    }];
}
